...
function CartPage() {
  ...
  return (
    <main className="mx-auto max-w-2xl px-4 py-6">
      ...
      <div className="mt-6 divide-y rounded-2xl border border-border bg-card">
        {items.map((i) => (
          <div key={i.variantId} className="flex gap-3 p-4">
            {i.image && <img src={i.image} alt={i.name} className="h-20 w-20 rounded-lg object-cover" />}
            ...
            <div className="flex flex-col items-end justify-between">
              <button onClick={() => remove(i.variantId)} className="text-muted-foreground hover:text-destructive">
                <Trash2 className="h-4 w-4" />
              </button>
              <div className="flex items-center gap-1 rounded-full border border-border">
                <button onClick={() => updateQty(i.variantId, i.qty - 1)} className="grid h-7 w-7 place-items-center"><Minus className="h-3 w-3" /></button>
                <span className="w-6 text-center text-sm">{i.qty}</span>
                <button onClick={() => updateQty(i.variantId, i.qty + 1)} className="grid h-7 w-7 place-items-center"><Plus className="h-3 w-3" /></button>
              </div>
            </div>
          </div>
        ))}
      ...
      <Button onClick={checkout} size="lg" className="mt-4 w-full bg-[#25D366] text-white hover:bg-[#1ebd5b]">
        <MessageCircle className="mr-2 h-5 w-5" /> Finalizar pelo WhatsApp
      </Button>
      <Button onClick={clear} variant="ghost" className="mt-2 w-full">Esvaziar carrinho</Button>
    </main>
  );
}
